...
- loop-rhythm-manager.js
  ...
  - **NEW (March 2026)**: Swap loop functionality - two-step workflow with visual feedback
  - **NEW (March 2026)**: Select existing loop from dropdown (no file duplication)
  - **NEW (March 2026)**: Duplicate rhythm set with real-time availability checking
  - Swap state tracking via global `swapState` variable
  - Purple border visual feedback for swap selection
  - Dynamic button states: Swap → Cancel (orange) → Swap Here (green)
...
### Rhythm Set Profile Learning System (NEW - March 2026)

**Status**: ✅ Production Ready - Analyzing 191 songs across 13 active rhythm sets

The rhythm set profile learning system transforms rhythm set assignment from static property matching to intelligent collaborative filtering using dynamic profiles built from actual song assignments.

**Core Components**:

- **scripts/core/build-rhythm-set-profiles.js** (276 lines)
  - Initial profile generation from existing song assignments
  - Analyzes mood, genre, taal, time signature, BPM statistics
  - Processes all songs grouped by rhythmSetId
  - Dry-run mode for validation
  - Usage: `node scripts/core/build-rhythm-set-profiles.js`

- **scripts/core/rhythm-set-profile-manager.js** (378 lines)
  - Profile update logic integrated with server.js
  ...
  - `handleRhythmSetChange()` - Maintains profile accuracy across assignments
  - Asynchronous updates (non-blocking API responses)

- **scripts/core/suggest-rhythm-assignments.js** (219 lines)
  - AI-driven song suggestions using profile-based scoring
  - 6-dimension scoring: mood, genre, taal, time signature, BPM, rhythm category
  - Configurable minimum score threshold
  - Filter for unassigned songs only (--unassigned flag)
  - Auto-apply mode for bulk assignments (--apply flag)
  - Generates markdown reports with match reasons
  - Usage: `node scripts/core/suggest-rhythm-assignments.js "Song Title" --min-score=45`
  - Example result: Found 34 similar songs for drum4-4_4 (90%+ accuracy)

- **scripts/core/check-rhythm-set-songs.js** (44 lines)
  - Quick utility to list songs assigned to a rhythm set
  - Shows song count and basic metadata
  - Usage: `node scripts/core/check-rhythm-set-songs.js "keherwa_1"`

**MongoDB Collections**:

- **RhythmSetProfiles Collection**
  - Dynamic profiles per rhythm set
  - Mood distribution (frequency counts)
  - Genre distribution
  - Taal distribution
  - Time signature distribution
  - BPM statistics (min, max, avg, median, samples)
  - Total song count and timestamps
  - Unique index on `rhythmSetId`

- **ProfileScoringConfig Collection**
...
Profiles automatically update on song CRUD operations via hooks in server.js:
...
- Asynchronous updates to avoid blocking API responses

**Scoring Algorithm**:

Profile matching uses weighted similarity scoring across 6 dimensions:
1. **Mood Match** (25%): Overlap between song moods and profile mood distribution
2. **Genre Match** (20%): Exact match or genre family similarity
3. **Taal Match** (20%): Exact taal/rhythm family match
4. **Time Signature Match** (15%): Exact time signature match (4/4, 3/4, etc.)
5. **BPM Similarity** (10%): Distance from profile BPM median (±20 BPM tolerance)
6. **Rhythm Category Match** (10%): Indian/Western/Others alignment

**Performance Metrics**:
- 13 active profiles covering all major rhythm families
- 191 songs analyzed across all rhythm sets
- Profile rebuild time: ~2 seconds
- Incremental update time: <100ms
- First test: 34 similar songs found with 90%+ accuracy
